export const ROUTES = {
    ROUTE_HOME: '/',
    ROUTE_LOGIN: '/auth/login',
    ROUTE_DASHBOARD: '/dashboard',
    ROUTE_NOTIFICATION: '/notification',
    ROUTE_CREATE_COMPANY: '/company/create',
    ROUTE_TABLE_COMPANY: '/company/table',
    ROUTE_MODIFY_COMPANY: '/company/modify',
    ROUTE_DETAIL_COMPANY: '/company/detail',
    ROUTE_PLACES_COMPANY: '/company/places',
    ROUTE_CREATE_USER: '/user/create',
    ROUTE_TABLE_USER: '/user/table',
    ROUTE_MODIFY_USER: '/user/modify',
    ROUTE_DETAIL_USER: '/user/detail',
    ROUTE_CREATE_CLIENT: '/client/create',
    ROUTE_TABLE_CLIENT: '/client/table',
    ROUTE_MODIFY_CLIENT: '/client/modify',
    ROUTE_CREATE_BRANCH: '/branch/create',
    ROUTE_TABLE_BRANCH: '/branch/table',
    ROUTE_MODIFY_BRANCH: '/branch/modify',
    ROUTE_MIGRATION: '/migration',
    ROUTE_CREATE_DOCUMENT: '/document/create',
    ROUTE_EXPIRED_DOCUMENT: '/document/expired',
    ROUTE_DOCUMENTS_USER: '/user/documents',
    ROUTE_CREATE_VEHICLE: '/vehicle/create',
    ROUTE_TABLE_VEHICLE: '/vehicle/table',
    ROUTE_MODIFY_VEHICLE: '/vehicle/modify',
    ROUTE_DETAIL_VEHICLE: '/vehicle/detail',
    ROUTE_CREATE_ATTENDANCE: '/attendance/create',
    ROUTE_TABLE_ATTENDANCE: '/attendance/table',
    ROUTE_USERS_ATTENDANCE: '/attendance/users',
    ROUTE_MODIFY_ATTENDANCE: '/attendance/modify',
    ROUTE_DETAIL_ATTENDANCE: '/attendance/detail',
    ROUTE_TABLE_PERMISSION: '/permission/table',
    ROUTE_TABLE_DEADLINES: '/deadlines/table',
    ROUTE_TABLE_WORKFORCE: '/workforce/table',
    ROUTE_LANDING_HOME: '/home/landing',
    ROUTE_ATTENDANCE_HOME: '/home/attendance',
    ROUTE_MYATTENDANCES_HOME: '/home/my-attendances',
    ROUTE_DOCUMENTS_HOME: '/home/documents',
    ROUTE_PERMISSION_HOME: '/home/permission',
    ROUTE_MEDICAL_HOME: '/home/medical',
    ROUTE_MEDICAL_DETAIL_HOME: '/home/medical/detail',
    ROUTE_GUIDE_HOME: '/home/guide',
    ROUTE_PROFILE_HOME: '/home/profile',
};
